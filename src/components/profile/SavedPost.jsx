...
const SavedPost = ({ userPost, loading, collections, collFilters }) => {
  const filters = ['all', 'hauls', 'lookbook', 'grwm', 'diy'];
  const col_filters = collFilters;
  const [currentFilter, setCurrentFilter] = useState('all');
  const [collectionsFilter, setCollectionsFilter] = useState(collFilters[collFilters.length - 1]);
  ...
  // const filteredCollections = collectionsFilter === 'general'
  //   ? collections
  //   : collections?.filter(post => post.coll_name === collectionsFilter);
  ...
  useEffect(() => {
    setOpenColz(false);
    console.log(collectionsFilter);
    let newlist = collections?.filter(post => post.coll_name === collectionsFilter)
    setFilteredCollections(newlist);
  }, [collectionsFilter])
        ...
        <div className='pb-[1rem] max-w-[90vw] mx-auto laptop:max-w-[70vw]'>
          ...
          {filteredPosts?.length > 0 ? (
            <div className='mb-[1rem] flex overflow-x-scroll gap-x-[10px] max-w-[90vw] mx-auto  laptop:max-w-[70vw] store__card'  >
              ...
              {filteredPosts?.sort((a, b) => b.timestamp - a.timestamp).map((post, i) => {
                ...
                return (
                  ...
                  <div key={i} className='w-[150px]' onClick={() => goToStore(post)}>
