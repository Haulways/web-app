...
class Editor extends React.Component {
    ...
    async generateThumbnail() {
        ...
        // Ensure the video has loaded metadata (duration)
        if (!video || video.duration === NaN) return;
    
        const canvas = document.createElement("canvas");
        canvas.width = 150; // Set the width of your thumbnails
        ...
        this.setState({ currentThumbnail: thumbnailDataUrl });
    }
    ...
    componentDidMount = () => {
        ...
        this.playVideo.current.addEventListener("timeupdate", () => {
        ...
        return this.state.timings.map((x, index) => (
            <div key={"grabber_"+index}>
                <div className="grabber start" style={{left: `${x.start / this.playVideo.current.duration * 100}%`}} onMouseDown={(event) => {
                    ...
                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" width="5" height="50" viewBox="0 0 10 14" xmlSpace="preserve">
            ...
            <div className="wrapper">
                <div className='video--container'>
                            ...
                            onClick={this.play_pause.bind(this)}
                                ...
                                currentTime={this.state.timings[0]?.start}
                                onTimeUpdate={(e) => {
                                    if (e.target.currentTime >= this.state.timings[0]?.end) {
                                        e.target.currentTime = this.state.timings[0]?.start; 
                                    }
                                }}
                            />
                   

                    <div className="player-controls--btn">
                        <button className="seek-start" title="Skip to previous clip" onClick={this.skipPrevious}>
                            <FontAwesomeIcon icon={faStepBackward} />
                        </button>

                        <button className="play-control" title="Play/Pause" onClick={this.play_pause.bind(this)} >
                            {this.state.playing ? <FontAwesomeIcon icon={faPause} /> : <FontAwesomeIcon icon={faPlay} />}
                        </button>

                        <button className="seek-end" title="Skip to next clip" onClick={this.skipNext}>
                ...
                <div className="controls">
                    <div className="player-controls">
                        <button className="settings-control" title="Reset Video" onClick={this.reset}>
                            <FontAwesomeIcon icon={faSync} />
                            Reset
                        </button>
                        
                        <button className="settings-control" title="Mute/Unmute Video" onClick={() => this.setState({ isMuted: !this.state.isMuted })}>
                            {this.state.isMuted ? <FontAwesomeIcon icon={faVolumeMute} /> : <FontAwesomeIcon icon={faVolumeUp} />}
                            {this.state.isMuted ? "UnMute" : "Mute"}
                        </button>

                        <button className="settings-control" title="Capture Thumbnail" onClick={this.captureSnapshot}><FontAwesomeIcon icon={faCamera} />
                            Thumbnail
                        </button>
                    </div>
    ...
    <img
        src={this.state.currentThumbnail}
        alt="Thumbnail"
        className="thumbnail"
    />
</div>
                ...
                <div className='trimmer--container'>
                    <button title="Add grabber" className="trim-control margined" onClick={this.addGrabber}>
                        Add trimmer
                    </button>

                    <button title="Delete grabber" className="trim-control margined" onClick={this.preDeleteGrabber}>
                        Delete trimmer
                    </button>

                    <button title="Save changes" className="trim-control" onClick={this.saveVideo}>
                        Save
                    </button>
                </div>
                ...
                {(this.state.imageUrl != "") ?
                    <div className={"marginVertical"}>
                        ...
                        <div className="controls">
                            <div className="player-controls">
                                {/* <button className="settings-control" title="Save Thumnail" onClick={this.downloadSnapshot}>
                                    <FontAwesomeIcon icon={faDownload} />
                                </button> */}
                                <button className="settings-control" title="Remove Thumnail" onClick={() => { this.setState({ imageUrl: "" }) }}>
                                    Remove
                                </button>
                            </div>
                        </div>
                ...
                <div className=' flex gap-x-3 overflow-x-scroll  store__card fixed bottom-0 bg-[#fff] p-[10px] w-full justify-center'>
                    {this.props.selectedfile.map((file, index) => (
                        <div key={index} className={`videos--tobe--edited ${index === this.state.activeFileIndex ? 'active' : ''}`} onClick={() =>
                            this.setState({ activeFileIndex: index })
                        }>
                            {file && file.type.includes('image') ? (
                                <img src={URL.createObjectURL(file)} alt="" className="shareImg" />
                            ) : (
                                <video src={URL.createObjectURL(file)} className="shareVideo" muted playsInline={true} controls={false} controlsList="nodownload" />
                            )}
