...
interface AdAnalyticsContextProps {
  totalImpression: StatisticsCardData;
  conversion: StatisticsCardData;
  budgetRemaining: StatisticsCardData;
  totalSpent: StatisticsCardData;
...
const AdAnalyticsContext = createContext<AdAnalyticsContextProps | null>(null);

export const AdAnalyticsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const totalImpression = useMemo(() => {
    ...
    return {
      ...
      isNegative,
      total: commaSeperatedNumberFormat(salesAmount),
    };
...
export const useAdAnalyticsContext = () => {
  const context = useContext(AdAnalyticsContext);

  if (context === null) {
    throw new Error(
      "useAdAnalyticsContext must be used within an AdAnalyticsProvider",
    );
  }
